...
def test_blast_radius_under_100ms(tmp_path: Path):
    ...
    # `# query_ms:` is emitted on stderr (v0.2.0+ keeps stdout strictly JSON);
    # merge stderr into stdout for parsing.
    proc = subprocess.run(
    ...
    line = next(
        (row for row in combined.splitlines() if row.startswith("# query_ms:")), None
    )
    assert line is not None, (
        f"code-map did not emit a `# query_ms:` line. output was:\n{combined}"
    )
